...
public class WelcomeOlfactiveView extends LinearLayout {
    ...
    private void init(Context context, Runnable onNext) {
        ...
        List<String> selectedCategories = Arrays.asList(olfactive.split(", "));

        // Gọi hàm riêng để vẽ biểu đồ
        setupRadarChart(context, selectedCategories,olfaciveChar, Color.parseColor("#000000"));
    ...
    // Hàm vẽ radar chart
    public void setupRadarChart(Context context, List<String> selectedCategories, RadarChart olfactiveChart, int color) {
        List<String> allCategories = Arrays.asList("Aquatic", "Woody", "Powdery", "Citrus", "Fruity", "Herbal", "Floral");

        List<RadarEntry> entries = new ArrayList<>();
        for (String category : allCategories) {
            entries.add(new RadarEntry(selectedCategories.contains(category) ? 2f : 1f));
        }
