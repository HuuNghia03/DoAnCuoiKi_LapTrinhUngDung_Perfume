...
import com.example.perfume.adapter.ScentAdapter;
import com.example.perfume.entity.ScentEntity;
            ...
            public void onCardSwiped(Direction direction) {
                ...
                if (index >= 0 && index < scentList.size()) {
                    if (direction == Direction.Right) {
                        String note = scentList.get(index).getNote();
                        // Nếu đã có trong map thì +1, chưa có thì đặt = 1
                        likedCounts.put(index, note);
