...
public class OrderFragment extends Fragment {
    ...
    private View createOrderView(OrderWithItems order) {
        ...
        tvOrderTotal.setText("Total price (" + totalProducts + " product" + (totalProducts > 1 ? "s" : "") + "): $ " + String.format(Locale.US,"%.2f", order.order.getTotalPrice()));
        ...
        orderView.setOnClickListener(v -> {
            OrderDetailFragment detailFragment = new OrderDetailFragment(order.order.getOrderId());
            FragmentTransaction transaction = getParentFragmentManager().beginTransaction();
            ((HomeActivity) getActivity()).setDetailFragment(detailFragment);
